...
void apply_jet_colormap_host(const float* gray_image, float* rgb_image,
  ...
  apply_jet_colormap<<<grid_dim, block_dim>>>(gray_image, rgb_image, width,
                                              height);
...
void apply_jet_colormap_wrapper(const float* h_gray_image, float* h_rgb_image,
  ...
  cudaMemcpy(d_gray_image, h_gray_image, width * height * sizeof(float),
  ...
  apply_jet_colormap<<<grid_dim, block_dim>>>(d_gray_image, d_rgb_image, width,
                                              height);

  // Copy RGB image data back to the host
  cudaMemcpy(h_rgb_image, d_rgb_image, width * height * 3 * sizeof(float),
